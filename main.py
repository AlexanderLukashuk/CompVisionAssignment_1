...
def softmax_regression(input_dim, num_classes, alpha=0.1, num_epochs=20):
...
class SoftmaxRegressionEstimator(BaseEstimator):
    def __init__(self, input_dim, num_classes, alpha=0.1, num_epochs=20):
...
softmax_scores = cross_val_score(SoftmaxRegressionEstimator(X_train.shape[1], len(np.unique(y_train)), alpha=0.1, num_epochs=20), X_train, y_train, cv=cv, scoring='accuracy')
...
best_softmax_model = SoftmaxRegressionEstimator(X_train.shape[1], len(np.unique(y_train)), alpha=0.1, num_epochs=20)
